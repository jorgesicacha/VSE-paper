...
#### Distance ####
setwd("/Users/jorgespa/Documents/Research/Special Issue Paper/Codes")
pts <- shapefile("distpoints.shp")
x.pred1 <- seq(min(pts@coords[,1]),max(pts@coords[,1]),length.out = 1000)
y.pred1 <- seq(min(pts@coords[,2]),max(pts@coords[,2]),length.out = 1000)
locs.pred1 <- expand.grid(x.pred1,y.pred1)
locs.pred1 <- SpatialPoints(locs.pred1)
setwd("/Users/jorgespa/Documents/Research/Special Issue Paper/Codes/Simulation")
distroad.region <- read.table("distpoints_roadshed.txt",sep=";",dec=',',header=T)
datareview2 <- SpatialPointsDataFrame(locs.pred1@coords,data=data.frame(NEARDIST=distroad.region$NEAR_DIST),proj4string = newcrs)
r.dist <- raster(datareview2)
r1.dist<-disaggregate(r.dist, fact=res(r.dist)/.5)
dist_hed <- rasterize(datareview2@coords,r1.dist,field=datareview2$NEARDIST,fun=max,na.rm = T)
