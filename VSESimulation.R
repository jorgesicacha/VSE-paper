...
simulation <- function(pars,thinpar,zetap,BB){
  beta0 <- pars[1];beta1 <- pars[2];sigma2x <- pars[3];range <- pars[4];zeta <- thinpar;zetap <- zetap;B=100
  lg.s.c <- list()
  results <- list()
  naive <- list()
  vse <- list()
  ...
  for(k in 1:BB){
    ...
    ## Step 5: Thin the PP based on the detection function
    ...
    for(m in 1:length(zeta)){
      ...
      thinprob.d <- exp(zeta[m]*dist.obs0)
      ...
      rownames(points.pred) <- seq(1,nrow(locs.pred@coords))
      
      MeshPoints <- SpatialPointsDataFrame(points.mesh ,data= data.frame(ID=1:nrow(points.mesh)),proj4string = newcrs)
      PredPoints1 <- SpatialPointsDataFrame(locs.pred1@coords,data= data.frame(ID=1:nrow(locs.pred1@coords)),proj4string = newcrs)
      
      ## Distances to road ##
      dist2road.pred <- extract(rast.dist,points.pred)
      dist2road.mesh <- extract(rast.dist,points.mesh)
      dist2road.obs <- extract(rast.dist,points.obs)
      dist.obs <- -0.5*(dist2road.obs)^2
      dist.pred <- -0.5*(dist2road.pred)^2
      dist.mesh <- -0.5*(dist2road.mesh)^2
      
      
      ## Validating sizes ##
      datareview2 <- SpatialPointsDataFrame(rbind(points.mesh,points.obs,points.pred),data=data.frame(NEARDIST=c(dist2road.mesh,dist2road.obs,dist2road.pred)),proj4string = newcrs)
      indxs <- over(SpatialPoints(datareview2@coords,proj4string = newcrs),norway)
      indxs1 <- which(!is.na(indxs)) 
      datareview3 <- SpatialPointsDataFrame(rbind(points.mesh,points.obs,points.pred)[indxs1,],data=data.frame(NEARDIST=c(dist2road.mesh,dist2road.obs,dist2road.pred)[indxs1]),proj4string = newcrs)
      r <- raster(datareview3)
      r1<-disaggregate(r, fact=res(r)/2.9)
      covrast <- rasterize(datareview3@coords,r1,field=datareview3$NEARDIST,fun=max,na.rm = T)
      ...
      cov.pred <- extract(rad_hed1,points.pred)
      ...
      results0[[m]] <- data.frame(real.bo=beta0,naive.b0=naive.model$summary.fixed$`0.5quant`[1],prop.b0=vse.model$summary.fixed$`0.5quant`[1],real.b1=beta1,naive.b1=naive.model$summary.fixed$`0.5quant`[2],prop.b1=vse.model$summary.fixed$`0.5quant`[2],real.b2=zeta[m],naive.b2=0,prop.b2=vse.model$summary.hyperpar$`0.5quant`[3],real.range=range,naive.range=naive.model$summary.hyperpar$`0.5quant`[1],prop.range=vse.model$summary.hyperpar$`0.5quant`[1],real.sd=sqrt(sigma2x),naive.sd=naive.model$summary.hyperpar$`0.5quant`[2],prop.sd=vse.model$summary.hyperpar$`0.5quant`[2],naive.dic=naive.model$dic$dic,prop.dic=vse.model$dic$dic,naive.waic=naive.model$waic$waic,prop.waic=vse.model$waic$waic,naive.cpo=-sum(log(naive.model$cpo$cpo)),prop.cpo=-sum(log(vse.model$cpo$cpo))) 
...
simu_results1 <- simulation(as.numeric(pars1),thinpar=c(0,1,8,16),zetap=rep(1,4),BB=2)
simu_results2 <- simulation(as.numeric(pars1),thinpar=c(8,16),zetap=c(3.5,5),BB=2)
